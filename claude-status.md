...
_Updated: 2026-06-22 (session 62 — **Metering step 1 built + deployed (METERING_PAYMENTS.md §12 step 1 — the credits data model + idempotent $5 grant; no draw-down/gate/Stripe — those are later steps).** Migration `20260622000000_metering_credits.sql` adds two tables per §4: `athlete_credits` (live balance cache — `balance_cents`, `comped`, auto-reload + Stripe-customer columns) and `credit_ledger` (append-only audit — signed `amount_cents`, `balance_after_cents`, `kind grant|topup|debit|refund|adjust`). Cents only, RLS on / no policies (service-role only, matching the schema convention). The $5 grant is the `grant_signup_credit(athlete_id, amount=500)` RPC — writes ledger row + balance in one txn, **idempotent via a partial unique index `credit_ledger_one_grant_per_athlete on (athlete_id) where kind='grant'`** (a concurrent 2nd grant fails the index and rolls back the bump; a fast existence-check handles the common case). Fires at v3 **onboarding-complete** (`finishOnboarding` in `engine/router.ts`, wrapped so a billing hiccup never blocks onboarding — David confirmed this over the link-step) and reused by an in-migration backfill of all existing athletes. **Comp: David only** (real chat `8940829310` + the negative-chat test/group row) `comped=true`; the 6 friends carry `comped=false` + the $5 grant so step 2's draw-down exercises live (David confirmed). TS wrapper `src/server/billing/credits.ts` (`grantSignupCredit`) + mock contract test; `db-types.ts` hand-edited (no local Supabase). **Verified against prod:** both tables exist; all 8 athletes have exactly 1×$5 grant, `balance==ledger==500`, in-sync; re-calling the RPC returns false / no double-write (app idempotency); a raw 2nd grant insert is rejected `23505` (DB guard). 4 unit tests + `npm run build` green. **Vercel-only — `supabase db push` + commit + push, no `fly deploy`.** ⚠️ Found uncommitted **worker/** changes from another session (`run-agent.ts`, `send.ts`, `system-prompt.ts`, `coach.md`, new `proposal.ts`) — left untouched, staged only my files. **Next: step 2 — draw-down (`worker/run-agent.ts` TODO(#12)) + the $0 pre-run gate.** Prior session below.)_

_(session 61 — **Auto-pause on inactivity built + deployed (METERING_PAYMENTS §10.5 — ships the pause primitive first, ahead of the rest of the metering doc; no dependency on credits/Stripe/the $0 gate).** Migration `20260614000000_athlete_pause_columns.sql` adds `athletes.paused_at` + `pause_reason`. The daily enqueue cron (`src/app/api/cron/daily-checkin/route.ts`) gains a `paused_at != null` skip filter — which is also what makes the notice fire exactly once — plus an inactivity scan: one bounded `messages` query for inbound (`direction='in'`) in the last 10 days; an onboarded, non-test athlete not in that set AND created before the window (the `created_at` floor = new-athlete grace) is auto-paused — set `paused_at`/`pause_reason='auto_inactivity'`, send ONE static notice with a `resume:auto` button via `pause.ts::sendAutoPauseNotice` (straight through the prod bot, NOT an agent run), no enqueue. Resume two ways: the button (`handleResumeAuto` in bot.ts — clears the pause, collapses the button, confirms, enqueues today's check-in) and any inbound message (`clearAutoInactivityPause` in `handleInboundText` + the callback router). The inbound path is gated on `pause_reason='auto_inactivity'` so a future **manual** `/pause` (§10 vacation) does NOT resume on an ad-hoc message — same column, opposite inbound behavior. Pure `isInactive` extracted + unit-tested. Cron carries a `?dryRun=1` / `AUTO_PAUSE_DRY_RUN` guard that reports pause candidates without writing or sending. **Rollout (David chose migrate + push w/ dry-run on):** migration applied to prod via `supabase db push`; `AUTO_PAUSE_DRY_RUN=1` set in Vercel production; commit + push (Vercel-only, no `fly deploy`). Dry-run probe against prod returned `{paused: [], dryRun: true, enqueued: 6}` — the path runs live, no friend qualifies today, nothing written/sent. **Verified: 1009 tests (full suite; new `pause.test.ts` + expanded cron route test), typecheck/lint/build green.** Left for David: flip dry-run OFF to go live (remove the `AUTO_PAUSE_DRY_RUN` env var or set `0`, then redeploy); notice copy is a draft for his voice pass. §10 manual `/pause`/`/resume` now reduces to a thin wrapper over this primitive. Prior session below.)_
