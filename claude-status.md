...
**In flight (uncommitted, behind the `ONBOARDING_V3` env flag — off leaves v2 unaffected):** onboarding v3, a slot-filling conversational intake replacing v2's button state machine (`Specs/ONBOARDING_V3.md`). W1 (slot foundation), W2 (per-turn Sonnet engine), W3 (orientation/openers hardening, recap polish, `/edit_profile` menu + known-gaps walk), a **W2 hardening pass** (session 39 — the deterministic-confirm loop + stale `goal_date`, both hit live 2026-06-05), **V3-W7** (session 40 — the non-race `coach.md` branch: a 3-way committed/intended/no_race coaching posture + race-only known-gap suppression for keep_fit athletes), and **V3-W8** (session 41 — the uncatalogued-goal pocket + deterministic distance derivation: an oversize/non-catalog goal is detected in code, acknowledged, offered a marathon-proxy with consent, and the athlete's words stored for the coach; confirmed-race `distance_mi` buckets in code, not the model) are built and unit-verified (637 tests green). W0/W4/W6 already shipped (cutover live; v3 is the default flow). **V3-W7 is not behind the v3 flag** — it changes the live worker coach for any athlete whose `goal_state` is `day_to_day`/`intended`, so it ships with a `fly deploy`, not just a Vercel push. Still untested live (no Sonnet run); the W2 migration isn't applied locally yet; `/edit_profile` needs adding to the BotFather menu. Detail in the session-41/40 blocks above. Remaining: W5 (eval harness — launch gate), then the ultra catalog (`Specs/ULTRA_SUPPORT.md` U1 — graduates pocketed goals to real plans).
