...
**Onboarding v2 — W4 (plan preview + adjust loop), now that the W3 engine is built.** W3 is done (session 26): six templates, the renderer/selector/validator, and dual-layer caps, all tested green. W4 wires `selectPlan → renderPlan` into the flow, inserting the **B1 preview** between injury-check and enrichment: render the plan after onboarding, show "here's your starting plan… `[Looks good]` / `[Adjust it]`", persist a `plan_versions` row, and route `[Adjust it]` to the worker coach (a `tg_message`-style job seeded "athlete wants to adjust the just-generated plan") for conversational customization on top of the rendered plan. Then next-actions (Add to calendar / Adjust / Done).

**W4 prerequisite — the snapshot adapter (camel/snake drift).** The selector consumes `FitnessSnapshotInput` (camelCase, declared structurally in `selector.ts` so `lib/` doesn't depend on `server/`); the real producer `getFitnessSnapshot` (`src/server/strava/activities.ts`) returns `StravaFitnessSnapshot` (snake_case: `recent_weekly_mileage_mi`, `suggested_days_per_week`, `dominant_long_run_weekday`, `road_trail_mix`, …). They are a deliberate decouple, not an oversight — but W4 must add the snake→camel adapter at the wiring boundary (where the onboarding route calls `selectPlan`). The W3 engine + tests construct `FitnessSnapshotInput` directly, so nothing in `lib/` needs to change.

**Also for W4 / before friends onboard on v2:**

1. **Run W2 live end-to-end via the W0 harness** (`docs/testing-onboarding.md`) — staging bot + real Strava OAuth, exercising committed / A4b intended (day-to-day is "coming soon"). Assert `athlete_training_profile`, `races`/`injuries`, provenance-tagged memory, terminal `onboarding_state`. *(David is running this in parallel.)*
2. **Audit dormant old-section readers:** `buildTemplateValues`/`loadAthleteData` in `byo-plan.ts` and any worker coach-prompt regex still read old `athlete_profile.md` sections (`Schedule`/`Goals`/`Anything else`) + `extractNotesValue` that W2 no longer writes. Grep and reconcile before friends hit W3/W4. (`loadAthleteData` is now also used by `worker/system-prompt.ts` for `{{safety_caps}}` — it reads `goalRace.distance_mi`, which is fine.)

(Resolved in W3: the `no-timeline` intended athlete routes to an open-ended base+build block — `computeRenderParams` returns `totalWeeks: null` and the renderer emits a finite horizon, peak/taper deferred until a date binds.)
