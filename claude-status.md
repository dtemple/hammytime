...
- **Onboarding reflection + Nathan fix bundle (specced 2026-06-10, awaiting sign-off)**: the first post-cutover live onboarding (`transcripts/nspady_gmail.com.md` — sub-5 mile goal) exposed five bugs: no catalog **floor** (`deriveBucketFromMiles` maps 1 mi → `5k` silently, `stated`+confirmed — the W8 pocket only fires above 28 mi, so the mile became a 5K with no acknowledgment); an affirmed recap confirms nothing in code (five serial "Quick check" turns for already-recapped values → rubber-stamping); no future-date guard on `goal_date` ("September or later" → `2025-09-01`, past, confirmed as raw ISO); the "~1 weeks" preview horizon downstream of that; and `target_time` validated against the proxy bucket so "sub-5" evaporated. Beyond the bugs, the design finding: the ramble opener sets up "understood as a whole runner," then the slot machinery keeps one slot's worth — Nathan's other three threads (short-distance speed, strength/resilience, injury avoidance) were never mentioned again. **`Specs/ONBOARDING_REFLECTION.md`** carries the trace, the design (reflection beat mirroring the full ramble + honest boundary at minute two; `intents` portfolio persisted to the profile memory file and read by the coach; logistics reframe), and two session prompts: **R1** (fix bundle, independently shippable, absorbs T-9) and **R2** (the shape). Open: floor value (DRAFT 2.5 mi), short-side proxy (`5k`), reflection copy, whether R1 ships ahead of sign-off. SPEC.md/CLAUDE.md untouched pending sign-off.
...
**General Fitness — GF-W3 (race→fitness rollover).** GF-W2 shipped session 56 (v0.7.33) and is now live for a real athlete: David decided Anjie's intention is general fitness, and her v2-era row was fixed to `day_to_day`/`day_to_day`/`target_date=null` same day (GENERAL_FITNESS.md open question 6, decided) — her rendered prompt verified no_race, with her real 5 runs/week target. **One loose end: her v2-seeded `known_gaps.md` still carries a race-only `[open] target_time` line** — removal was proposed but denied as beyond the approved fix; David to remove it (or let the coach retire it) — and **watch her next 6:30am message** for the consistency-story lead landing well. Then **GF-W3** per the §7 sequencing: reuses W1's `extendPlanIfDue` machinery (already placed in `src/server/plan/` for that purpose) and is the other source of day_to_day athletes. `scripts/render-system-prompt.ts <email>` is the standing zero-write review tool for any `coach.md` / `system-prompt.ts` edit.
