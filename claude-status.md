...
**Prehab v2 — P3 only (P1+P2 built session 52; CHANGELOG v0.7.29).** Remaining steps, in order: (1) **David vets the science** in `worker/knowledge/prehab-principles.md` (§4 of `Specs/PREHAB.md` — cut anything contested, don't hedge); (2) flip the "PENDING SIGN-OFF" line in that file's header to the verified date, commit + push; (3) `fly deploy` (worker-only); (4) watch David's own transcript for ~a week against the §6 acceptance criteria (routine on its 2–3 days and no others; contextual items always name their cause; persisting signals repeat with acknowledgment; a plan reshuffle moves weekday mapping without dropping anchors; a second athlete's program reflects *their* injuries). No backfill — each athlete's first daily run post-deploy authors their program file.
