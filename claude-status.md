...
**General Fitness — David's call on Anjie's profile row, then GF-W3.** GF-W2 shipped session 56 (v0.7.33) but is live-but-dormant: prod has zero `goal_state='day_to_day'` athletes. Anjie — the one live keep_fit athlete — carries a v2-era `goal_type='race'` / `goal_state='intended'` / `goal_distance='keep_fit'` combo with a stale `target_date` (2026-07-30), so she coaches in `intended` mode ("a race in mind") and sees none of the no_race behavior, W7's or W2's. Recommended one-row fix (GENERAL_FITNESS.md open question 6): `goal_type='day_to_day'`, `goal_state='day_to_day'`, `target_date=null` — what v3 onboarding would have written. After the call: **GF-W3 (race→fitness rollover)**, which reuses W1's `extendPlanIfDue` machinery (already placed in `src/server/plan/` for that purpose) per the §7 sequencing — and is the other source of day_to_day athletes. `scripts/render-system-prompt.ts <email>` is the standing zero-write review tool for any `coach.md` / `system-prompt.ts` edit.
