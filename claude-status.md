...
- **Onboarding reflection (specced 2026-06-10; R1 SHIPPED session 57, R2 open)**: the first post-cutover live onboarding (`transcripts/nspady_gmail.com.md` — sub-5 mile goal) exposed the Nathan bug cluster (no catalog floor → silent mile→5K; affirmed recap confirms nothing → five serial Quick checks; no future-date guard → `2025-09-01`; "~1 weeks" preview; sub-5 dropped against the proxy bucket band) — **all fixed in R1 (session 57)**. The design finding stands: the ramble opener sets up "understood as a whole runner," then the slot machinery keeps one slot's worth — Nathan's other three threads (short-distance speed, strength/resilience, injury avoidance) were never mentioned again. **`Specs/ONBOARDING_REFLECTION.md`** carries the trace and the **R2** design (reflection beat mirroring the full ramble + honest boundary at minute two; `intents` portfolio persisted to the profile memory file and read by the coach; logistics reframe), still awaiting sign-off. Open for David: R1's DRAFT values (2.5 mi floor, `5k` proxy, 230–1500 s/mi envelope, 3-wk preview threshold), the new athlete-facing copy (short pocket, injury direct-ask, commit-refusal re-ask) + a staging-group pass, and R2 reflection copy. SPEC.md/CLAUDE.md untouched pending sign-off.
...
**Onboarding reflection — R1 follow-through, then R2.** R1 shipped session 57 (commit + push only — Vercel-side). Before friends onboard again: David's read of the DRAFT values + the three new athlete-facing strings (short-pocket offer, injury direct-ask, commit-refusal re-ask), and a staging-group pass (`docs/testing-onboarding.md`) — ideally replaying Nathan's mile ramble to watch the pocket + bulk-confirm live. Then **R2** (the reflection beat + `intents` portfolio, `Specs/ONBOARDING_REFLECTION.md` §2) once David signs off the spec; R1's floor and bulk-confirm were its prerequisites, so it's unblocked.
