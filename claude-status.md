...
_Updated: 2026-06-10 (session 59 — **R2 staging fixes built (CHANGELOG v0.7.35; findings in `ONBOARDING_REFLECTION.md` §2.4.2): David ran five live onboarding tests same day as the R2 deploy and the shape held — mirrors in all five, the 50-mile boundary + redo exactly per spec — with two misses, both diagnosed and fixed.** (1) **The mile enum-bypass:** the model filled `goal_distance: '5k'` directly instead of `goal_distance_mi: 1`, skipping the R1 catalog floor (it lives on the mi path) — no boundary, a 5K plan presented as the mile plan; "sub-5" also never extracted as target_time. Fix: short-side ENUM_RULES ("a mile" → goal_distance_mi: 1; never map to the nearest bucket in either direction) + a same-turn target_time extraction rule + a **deterministic cross-fire net** — the numeric backstop also fires when a model fill touches goal_distance against an already-held implausible target time, and the copy questions the *pairing* ("I had 5:00 as your goal time, but that doesn't fit a 5k — is the distance actually different…?"). Race-lookup buckets and typed pocket-accepts can't trip it (state writes / reconcile-first, both pinned by test). (2) **The volume-goal boundary (David's decision: acknowledge, can't-help-yet, redirect — NOT the ULTRA_SUPPORT §6-interim proxy pocket):** "100 miles every month" had been happily agreed to and committed as a plain keep_fit base, the number nowhere structured (prod-verified: no intent, no pocket, profile = Identity only). New `volume_goal {miles, period}` extraction signal; when it's the headline goal (newly stated, no race same-turn OR in state) the turn deterministically states the boundary + `[Keep me fit]`/`[Train for a race]` chips under the mirror, and blocks a same-turn generate; the clause always rides as an intent (dedupe gates the boundary to once; alongside a race it demotes silently). (3) **Preview truthfulness:** "No time goal locked in yet" was unconditional — now renders the locked target ("Aiming at 3:55:54"); "peaking at 8" gains "mi"; the intents line drops the over-promising "The plan carries those too". **Verified: 978 tests (31 new), typecheck/lint/build green. Vercel-only — commit + push, no `fly deploy`.** Flagged, not built: the pre-existing stale accepted-pocket pivot (Chase class — an accepted 44-mi pocket + a later race poisons the race row's distance_mi; fix sketched in CHANGELOG), a backstop attempts counter, full §6 volume support. **Copy (boundary, cross-fire, preview target line) is DRAFT for David's voice pass; real verification = staging replay of the mile ramble + "100 miles every month" — the routing half of the mile fix is prompt-side.** Prior sessions below.)_
