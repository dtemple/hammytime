...
_Updated: 2026-06-02 (session 25 — onboarding v2 W3: template-library shape scaffolded. Locked the 6-template shape (distance axis + tier-as-band + params/overlays), extended plan-schema for road/short workouts (tempo/intervals day types + pace fields), added the strength model + equipment known gap, and proposed starting safety-cap numbers. Type layer + selector/renderer interfaces + 1 filled example template in `src/lib/plan-templates/`; renderer bodies + the other 5 templates still to build. typecheck/lint/tests green. SPEC v0.7.9 logged.)_
...
- **Remaining W3 build:** finalize the cap numbers (approved-with-tuning), author the other 5 templates, implement the renderer + validator bodies, and the **dual-layer cap enforcement** — surface caps into the rendered plan's `agent_guidance` + the worker coach prompt (one source of truth) and make the coach advisory (warn + confirm + comply) on violations. Then W4 (preview/adjust/next-actions) wires it into the flow.
...
**Onboarding v2 — finish W3 (renderer + remaining templates), then W4 (preview + adjust loop).** W3's *shape* is scaffolded (session 25): types, selector, caps, one example template, known-gaps module, schema additions — all green, all in SPEC v0.7.9. The remaining W3 build, in order: (1) **finalize the safety-cap numbers** in `src/lib/plan-templates/caps.ts` (weekly ramp ≤12% or +3mi (whichever is greater), LR step ≤2/3mi, LR ≤50% of week, ≥1 easy day between hard, per-distance LR ceilings — David approved the ramp loosening + permit-but-discourage time_goal on marathon-finish for some_training); (2) **author the other 5 templates** (`marathon-performance`, `half-foundation`, `half-development`, `short-race`, `base-maintenance`) following the `marathon-finish` shape — note: `short-race` should likely *include* `time_goal` in `supportsOverlays` even for beginners (short races are pace-driven; the foundation-suppresses-time-goal rule is specifically about long-distance first-timers); (3) **implement the renderer + validator bodies** (`renderPlan`/`allocatePhases`/`buildWeeks`/`placeStrength`/`applyOverlays`/`validateSafety`) + `computeRenderParams`. Then W4 (plan preview B1 + adjust loop + next-actions, inserting the preview between injury-check and enrichment) wires it into the flow. Two things to do first/alongside:
