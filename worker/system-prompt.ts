...
// The "Protect the goal-race buildup" section — the floor under the safety caps.
// '' for a no_race athlete (no event to condition for); the full section for
// every race-ish mode (committed/intended/unknown). Same empty-substitution
// pattern as {{ease_in_context}}. The numbers it relies on live in the readiness
// signal (plan_drift.md, src/lib/plan-drift.ts), not here.
export function goalBuildupGuidance(mode: CoachMode): string {
...
export async function renderSystemPrompt(
  ...
  const values: Record<string, string> = {
    ...
    goal_buildup_guidance: goalBuildupGuidance(mode),
...
export function buildPrompt(
  ...
  if (source === 'daily_checkin') {
    // Sunday is the weekly big-picture beat, not a regular daily note. It points
    // at the "What a Sunday review looks like" section and the macro sources the
    // weekday notes deliberately skip (plan_drift.md's readiness headline + the
    // full plan). The athlete comes to expect it as their "Sunday review".
    const base =
      weekday === 'Sunday'
        ? `It's the morning of Sunday, ${date}. This is the weekly Sunday review, not a regular daily note — write it per the "What a Sunday review looks like" section of your instructions. Read strava_recent.json, plan_drift.md (its "# Race readiness" headline), the full marathon_training_plan.json, and the recent checkin_log.md first, then give the week-behind / big-picture / week-ahead review and end on a question that sets up the week.`
        : `It's the morning of ${weekday}, ${date}. Write today's coaching message: read the Strava file and the athlete's memory files first, then send a training-focused note for today.`;
