...
5. **Reversible within a section.** Every screen past the first in a section carries a `← Back` button, and a narrow set of typed cancel phrases ("never mind") on a text screen step back instead of being read as the answer. Editing an already-finished section still means `/restart` — in-section only, no mid-flow rollback of saved rows. (Built 2026-06-03; see W2.)
...
- **In-section back (added 2026-06-03).** A reserved `onb:back` callback, intercepted centrally in `handleOnboardingCallback` and dispatched to a new optional `OnboardingStep.handleBack(partial)`, steps one screen back within a section; each multi-screen step renders prior screens through a shared `screenFor(partial)` so prompt strings live once. Goal-setup gained explicit `distance`/`timeframe` sub_steps so screen identity is recoverable for Back. Text screens also treat a narrow cancel set (`isCancelPhrase` in `src/server/telegram/onboarding/back.ts`; excludes `none`/`no`/`skip`, which carry meaning here) as a back request — this fixes the reported trap where typing "None, never mind" at the injury body-part prompt was saved as the body part. Scope is in-section only: no cross-section editing, no saved-row rollback (`/restart` stays the full redo). Wired into goal-setup / training-shape / injury-check; profile-confirm, enrichment, and plan-preview keep their existing intra-step escapes.
