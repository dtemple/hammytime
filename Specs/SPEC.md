...
- Job queue: Vercel cron + `job_queue` (no Inngest, per cut #4). Every 30 min the cron _enqueues_ `daily_checkin` jobs for athletes in their 6:30–7:00 AM local window. The **Fly.io worker container** drains the queue with `FOR UPDATE SKIP LOCKED` (v0.7).
...
- Daily check-in delivery: a Vercel cron (`30 13 * * *` = 6:30 AM Pacific) enqueues one `daily_checkin` job per onboarded, non-test athlete (v0.7.18 — was single-athlete). The Fly worker drains the queue. Timing is one clock fire, not per-athlete-local — per-local 6:30 is deferred.
...
4. The agent's final message is the athlete-facing response — and the _only_ morning message. Render into Telegram-friendly markdown; send. (The wellness battery no longer follows automatically; it's `/checkin`-only — see v0.7.2 change-log. The shadow-bcc mirror to David was removed in v0.7.3.)
...
### 3.9 Onboarding (v0.7.12 — Telegram conversational, Strava-forward, template plan preview)

_v3 approved 2026-06-04 (slot-filling conversational intake, Sonnet, freeform-first) — see the v0.7.20 change-log entry and `Specs/ONBOARDING_V3.md`. The body below describes the built v2 flow; it is rewritten here as the v3 workstreams land, per the repo convention._

Onboarding runs entirely in Telegram on a step state machine (`src/server/telegram/onboarding/`; state in `athletes.onboarding_state` = step index + partial answers, so an athlete can drop off and resume). It replaced the old ~15-question script with a short structured core plus one optional freeform dump, and it shows a real plan before asking for anything extra ("payoff before enrichment"). Required and safety-relevant inputs are buttons or guarded parses; everything else is soft and deferred. Strava-derived values (name, sex, timezone, fitness snapshot) are shown back for confirmation, never blind-trusted into a safety-driving field. Full flow sketch + draft copy: `Specs/ONBOARDING_V2.md`.

Beats, in order (the built steps):

- **A0 · Link + welcome** (`bot.handleStart`) — `/start <link_token>` links `telegram_chat_id ↔ athlete_id`, then offers a `[Connect Strava]` URL button.
- **A1 · Strava + profile-confirm** (step 0) — the OAuth callback (`resumeAfterStrava`) derives name, sex, timezone, and an 8-week fitness snapshot from Strava; the athlete confirms the name.
- **A2 / A4 / A4b · Goal** (step 1) — race vs. day-to-day (the latter "coming soon" in v1). Committed race via `lookupRace` → confirm; or "no race yet" → distance + rough timeframe. Writes `athlete_training_profile` (goal_state `committed` / `intended`) and a `races` row when committed.
- **A3 / A5 / A6 · Training shape** (step 2) — experience tier (Strava-prefilled), run days/week, long-run day, all buttons.
- **A7 · Injury quick-check** (step 3) — `[All good]` or one light capture (body part + active/watch); writes `injuries`.
- **B1 · Plan preview** (step 4) — render + persist the template plan (§3.4) and show it with `[Looks good]` / `[Adjust it]` (+ `[No strength]` when strength sessions are included). The template version seeds both `baseline_version_id` and `current_version_id` (§3.4; v0.7.11 drift model). `[Looks good]` continues to enrichment; `[Adjust it]` hands the just-generated plan to the worker coach for conversational customization (the coach edits the working version, baseline preserved).
- **C1 · Enrichment dump** (step 5, terminal) — one optional freeform/voice message; inline Haiku extraction tags fields `stated` / `inferred` / `unknown` and echoes them back to confirm (never writes an inferred value into a safety field). On completion: the onboarding-complete David alert + the Phase D next-actions (`[Add to calendar]` / `[Adjust the plan]` / `[That's it for today]`).

Three goal states are modeled: **committed race** (dated countdown with a taper), **intended-with-date** (finite base+build, taper deferred until a real race binds), and **intended-no-date / keep_fit** (open-ended rolling block). Day-to-day coaching with no race at all is "coming soon" in v1. Nice-to-haves (exact age, target time, tune-up races, strength equipment, schedule constraints) are gathered in the dump or filled opportunistically by the daily coach: onboarding seeds a `known_gaps.md` memory file marking what's still unknown, and the coach asks one gap at the moment its answer changes a prescription, then writes it back and clears it (v0.7.15). They're never front-loaded as questions.

(The retired v0.3 path — a ~15-question deterministic script ending in a BYO-plan paste — is gone from the live flow; BYO onboarding-time utilities remain on disk as the deferred fallback per §3.4.)
