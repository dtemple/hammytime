...
- **Activity-triggered runs (shipped v0.7.18 — Option B).** Activity-create events enqueue a `tg_message` coaching run via `handleActivityCreate` (`src/server/strava/activity-trigger.ts`): resolve `owner_id` → athlete, guard onboarded/non-test, apply a 3h per-athlete cooldown, enqueue with a `/fresh_update`-style seed. The worker's hydrate re-fetches Strava live, so the agent sees the new activity with no persistence — the event stays a trigger, not a store. **Any** activity type fires it (the event payload omits sport type; gating to runs would cost a `GET /activities/{id}`). The cooldown holds down the `agent_runs`/credit drawdown this reopens (the proactive-send concern from v0.7.2) and makes the post-activity send defer to the morning daily. Deferred: "short"/custom post-activity copy distinct from the morning note.
