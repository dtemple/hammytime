...
- **`/resume`** — clears the pause **and enqueues today's check-in immediately** (idempotent on the `daily-{athlete_id}-{date}` key, so a resume on a day the run already fired is a no-op — no double-charge), so coming back feels live rather than "starts tomorrow." Costs one agent run per resume — accepted. Bot confirms. Not-paused replies idempotently.
- **While paused:**
