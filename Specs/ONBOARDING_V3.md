...
_Status: **built and live** (2026-06-05). Signed off 2026-06-04; W0–W4, W6, W7, W8 shipped (v3 is the default flow behind the `ONBOARDING_V3` env flag, on in prod). **Only W5 — the eval harness — remains, deferred by decision** (we come back to it before opening to more friends). Supersedes the Phase-A/Phase-C button-forward shape in `ONBOARDING_V2.md`. `SPEC.md` §3.9 and `CLAUDE.md` §4 now describe v3 as current; see the v0.7.23 change-log entry. Governance note at the end._
