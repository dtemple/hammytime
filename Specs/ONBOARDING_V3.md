...
_Status: **built and live** (2026-06-05). Signed off 2026-06-04; W0–W4, W6, W7, W8 shipped. v3 is the default flow — `ONBOARDING_V3` is now a kill-switch back to v2 (on unless explicitly `false`/`0`/`off`), not an enable-switch. A full live Sonnet staging pass has run and `/edit_profile` is in the BotFather menu. **Only W5 — the eval harness — remains, deferred by decision** (we come back to it before opening to more friends). Supersedes the Phase-A/Phase-C button-forward shape in `ONBOARDING_V2.md`. `SPEC.md` §3.9 and `CLAUDE.md` §4 now describe v3 as current; see the v0.7.23 change-log entry. Governance note at the end._
...
- **Compute derived values.** When the athlete gives a pace ("10 minute miles for a marathon") the engine computes the implied finish (~4:22) rather than dropping the figure or guessing. When they give a finish, it can state the implied pace. This is reasoning the regex parser (`durations.ts`) can't do and the LLM extractor can. _(v0.7.54: the pace→finish arithmetic now lives in **code**, not the model — the extractor emits `goal_pace_sec_per_mi` and the router calls `paceToFinish`; the LLM no longer does the multiply, which drifted in the structured fill and stored a wrong-but-in-band `target_time` the backstop couldn't catch. Same "model parses, code does the safety-critical math" shape as `goal_distance_mi` → `deriveBucketFromMiles`.)_
...
- **V3-W7 · Non-race coaching branch in `coach.md` · S/M. ✅ Built and verified (2026-06-05), uncommitted; ships with a worker `fly deploy`.** With general-fitness athletes now routed in (Opener 1), the daily coach can no longer assume a dated race. A conditional in `worker/prompts/coach.md`: when `goal_state` is `keep_fit` / no-race, drop the race/taper/peak/countdown framing and the goal-pace-session logic, and switch to a consistency/base narrative; suppress the known-gaps that only pay off against a race (`target_time`, `tune_up_races`). This is the floor that keeps a no-race athlete's *daily* experience from being race-framed and wrong — the deeper non-race progress model can come later. Pulled into v3 scope because plan-gen already renders `keep_fit`, so the coaching message is the only place the no-race athlete still degrades. _Done: the worker coach had **no `goal_state` awareness** (`loadAthleteData` read only `races` → `goalRace`), so this first plumbed `getTrainingProfile` into `loadAthleteData` (`trainingProfile` on `LoadedData`). `worker/system-prompt.ts` derives a 3-way `coachMode` — **committed** (real race) / **intended** (`goal_state='intended'`, distance picked, no race named — gets a "lock a race when the timing's right" line) / **no_race** (`day_to_day`/keep_fit) / unknown (legacy → prior behavior) — and feeds new `coach.md` placeholders (`coach_title`, `coach_mission_line`, 3-way `goal_race_line`, `target_time_gap_guidance`, `known_gaps_examples`) through the existing `{{var}}` substitution (no new templating engine). Gap suppression keys on a derived `raceOnlyGapKeys()` exported from `slots/schema.ts` (single source of truth = the slots' `raceOnly` flag); `renderKnownGapsFromFilled`/`seedKnownGapsFromFilled` gain `excludeRaceOnly`, passed from `commit.ts` and the `/edit_profile` gap-walk (`router.ts`). 617 tests green (10 new incl. a render-safety test asserting no residual `{{` for all three modes). **Not behind the v3 flag** — affects the live worker coach for `day_to_day`/`intended` athletes. Live check (keep_fit daily run reads consistency/base-framed) gated on the broader v3 staging pass. Plan: `~/.claude/plans/please-read-onboarding-v3-sparkling-giraffe.md`._
- **V3-W8 · Uncatalogued-goal pocket + deterministic distance derivation · S/M (added 2026-06-05). ✅ Shipped (2026-06-05).** The two engine behaviors from the second live day (§1.2, §5.2, §5.3): the code-detected `out_of_catalog` goal state with the acknowledge → proxy-with-consent → store-the-words behavior, and bucket-from-`distance_mi` derivation for confirmed races (out-of-range → the pocket). Prerequisite for `ULTRA_SUPPORT.md` U1, which widens the catalog underneath it; independent of W4–W6 and shippable in either order. (The 2026-06-05 confirm-loop and stale-date fixes are bug fixes against W2, not a workstream — `Specs/V3_HARDENING_PROMPT.md`.) _Done: new `deriveBucketFromMiles` (`engine/numeric.ts`) buckets a confirmed race's `distance_mi` in code (`resolveRace`, `stated` provenance so it skips the redundant distance-confirm that caused the Western States loop); the model surfaces a non-standard stated distance via a new `goal_distance_mi` extractor field instead of guessing; >28 mi → null → the pocket (new `engine/pocket.ts`: `out_of_catalog` state on `V3OnboardingState`, schema_version 1→2; acknowledge → marathon-proxy with consent chips + a router fast path → on accept, proxy `goal_distance` + a `North-star goal` memory section + the real `distance_mi` on the race row). Decline re-offers and leaves the goal open; the recap shows the real goal, not the proxy. `mergeFills` also resets a derived `goal_distance` on a goal-race change. **Strict scope: mechanism only against the current 5-bucket catalog** — no new buckets, no migration, no templates (those are `ULTRA_SUPPORT.md` U1). 637 tests (+20) green; Vercel-only (no worker change); commit `603f3fd`._
