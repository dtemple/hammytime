...
6. Expiry + one-outstanding handling (folded into 2 + 4) — **built 2026-06-09**. Expiry was originally passive — enforced at tap, no sweep cron — with the window `min(end of affected week, 72h)` (see the closed open question below). A daily sweep was **added 2026-06-22** (see the "Proposal awareness" section); expiry is now both enforced at tap and cleaned up daily. Error/budget-stopped runs stage the candidate but suppress the keyboard, so the fallback message never pairs with a confirm button.
...
## Inbound routing — read the active version, not the newest (2026-06-22)

A latent bug in the original design: the propose flow stages a `proposed` `plan_versions` row as the **newest** row without moving `current_version_id`. The post-onboarding inbound router in `bot.ts` resolved the plan version by `order('created_at', desc).limit(1)` and required `status === 'active'` to enqueue — so an unconfirmed proposal (the newest row) made every inbound message fall through to the canned "onboarding is complete" reply. The message was never persisted or enqueued: the athlete went mute to the coach, and with no persisted inbound was also on track to be auto-paused for inactivity. Two of seven athletes were hit before it was caught.

Fix: the inbound router now filters the version query to live statuses (`active`, `awaiting_paste`) instead of taking the newest row, so a pending proposal or superseded/discarded history is ignored and routing follows the working plan. Web/bot change (`git push`), shipped 2026-06-22.

## Proposal awareness, single-confirm, cancel + expiry sweep (B, 2026-06-22)

The original flow left the coach blind to an outstanding proposal and conflated two confirm steps. In practice an athlete would be asked to confirm twice: the coach asked "want me to update your calendar?" in prose **and** the file edit auto-appended a Yes/No button. The coach also couldn't see a pending proposal on its next run, so it would re-propose (or stack a fresh proposal right after a confirm), and a never-tapped proposal lingered indefinitely. This batch closes those gaps. Worker + web change (`git push` + `fly deploy`).

- **Awareness.** `worker/proposal.ts` `loadPendingProposal` loads the outstanding `proposed` version, diffs it against the active plan, and renders a per-day summary. `renderSystemPrompt` injects it as a `{{pending_proposal_context}}` block (alongside the run's other situational blocks), telling the coach a change is already waiting on the tap and what it changes — so it doesn't re-propose or re-ask. An expired proposal is treated as none.
- **Single confirm.** The button **is** the confirmation. `coach.md` §"Changing the plan — the button is the confirmation" is rewritten: the coach edits the file, states the change declaratively, and lets the button ask — it never writes "want me to update?" and then drops a button under it. The one carve-out is a safety-cap call, where the coach gets a prose yes *before* editing for the risk decision; even then it doesn't re-ask about the calendar.
- **Cancel.** The coach drops a pending proposal without staging a replacement by writing a `.cancel_pending_change` sentinel into its folder (a dotfile, so `syncBack` skips it and the plan-edit hook ignores it — no custom MCP tool, per the v0.7 scope lock). `worker/run-agent.ts` detects it post-run (only when no fresh proposal was staged this run), calls `discardPendingProposal`, and resolves the stale button. The sentinel name is a single shared constant (`CANCEL_SENTINEL`) surfaced to the coach in both the awareness block and `coach.md`.
- **Expiry sweep.** `src/server/telegram/proposals.ts` `sweepExpiredProposals` discards every past-expiry proposal and edits its stale button. Wired into the daily-checkin cron (the only Vercel-side cron), independent of the auto-pause dry-run. This is the cleanup half; passive at-tap enforcement still stands.

### Open / deferred

- **Expiry is enforced in two places.** The sweep discards expired rows; `loadPendingProposal` independently treats an expired proposal as absent. Both are idempotent and safe, but the "expiry is final" invariant is split across two app-layer checks, and a row lingers until the next sweep. A deeper fix would make expiry a DB-level invariant — e.g. a `status='expired'` transition via trigger/RPC, so `loadPendingProposal` filters on status rather than comparing timestamps and the cron's only job is the Telegram message edit. Deferred (needs a migration); flagged as tech debt.
