...
- **Telegram-only onboarding.** The bot drives all onboarding in chat. There is no web onboarding flow. The web app surfaces sign-up — an allowlist check that branches to a Telegram deeplink (invited) or a waitlist capture (not invited, v0.7.17) — and nothing more on that path. **Onboarding v3 (approved 2026-06-04, SPEC v0.7.20; in progress) replaces v2's button-forward state machine with a slot-filling conversational intake — one global slot schema filled from freeform text/voice on Sonnet, chips for ≤3-option answers, an orientation + ready gate, and an `/edit_profile` menu. See `Specs/ONBOARDING_V3.md`. Telegram-only and Strava-required are unchanged.**
