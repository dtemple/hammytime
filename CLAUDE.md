...
- **Telegram-only onboarding.** The bot drives all onboarding via a conversational state machine. There is no web onboarding flow. The web app surfaces sign-up — an allowlist check that branches to a Telegram deeplink (invited) or a waitlist capture (not invited, v0.7.17) — and nothing more on that path.
