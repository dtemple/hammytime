...
function makeHaikuResponse(input: AnyMock) {
  return {
    ...
    content: [{ type: 'tool_use', name: 'parse_past_race', input }],
  };
}
...
async function step(text: string, partial: Record<string, unknown>) {
...
describe('tune-up cap', () => {
  ...
  it('warns at 4 tune-ups', async () => {
    vi.mocked(lookupRace as AnyMock).mockResolvedValue(
      foundResult({ canonical_name: 'Local Half' }),
    );
...
describe('past_race parsing', () => {
  ...
  it('re-asks once on Haiku parse failure', async () => {
    const createMock = vi.fn().mockResolvedValue(makeHaikuResponse({ garbage: true }));
    vi.mocked(anthropicClient as AnyMock).mockReturnValue({
      messages: { create: createMock },
    });
