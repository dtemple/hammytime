...
const timezoneQuestion: Question<string> = {
  key: "timezone",
  prompt:
    'What\'s your timezone? (e.g. PST, EST, America/Denver, or a city like "Chicago")',
  parseReply(text) {
...
const daysPerWeekQuestion: Question<number> = {
  ...
  parseReply(text) {
    ...
    if (n < 3 || n > 7)
      return { ok: false, error: "That needs to be between 3 and 7 days." };
    return { ok: true, value: n };
  },
};
...
const hoursPerWeekQuestion: Question<number> = {
  ...
  parseReply(text) {
    ...
    if (n < 3 || n > 20)
      return { ok: false, error: "That needs to be between 3 and 20 hours." };
    return { ok: true, value: n };
  },
};
