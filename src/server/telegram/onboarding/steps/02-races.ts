...
function parseTerrain(text: string): { ok: true; value: string } | { ok: false; error: string } {
  ...
  if (v.includes('trail') || v.includes('mountain') || v.includes('dirt'))
    return { ok: true, value: 'trail' };
...
function handleConfirm(text: string, p: Step2Partial, ctx: RaceContext): StepHandleResult {
  const v = text.trim().toLowerCase();
  ...
  if (v === 'no' || v === 'n' || v === 'wrong' || v === 'none' || v === 'manual' || v === 'skip') {
...
function handleManualDate(text: string, p: Step2Partial, ctx: RaceContext): StepHandleResult {
  const v = text.trim().toLowerCase();
...
function handleManualDistance(text: string, p: Step2Partial, ctx: RaceContext): StepHandleResult {
  const field = manualField(ctx);
  const manual = p[field] ?? { name: 'unknown' };
...
function handleManualElevation(text: string, p: Step2Partial, ctx: RaceContext): StepHandleResult {
  const field = manualField(ctx);
  const manual = p[field] ?? { name: 'unknown' };
  const v = text.trim().toLowerCase();
...
function handleManualTerrain(text: string, p: Step2Partial, ctx: RaceContext): StepHandleResult {
  const field = manualField(ctx);
  const manual = p[field] ?? { name: 'unknown' };
  const v = text.trim().toLowerCase();
...
const PAST_RACE_TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      finish_time_seconds: {
        type: 'number',
        description: 'Finish time in seconds, or null if unknown',
      },
...
async function racesHandleMessage(
  ...
  switch (p.sub_step) {
    ...
    case 'tuneup_loop': {
      ...
      return handleRaceName(v, athleteId, { ...p, tuneup_current_name: v }, 'tuneup');
    }
...
async function readStep1Target(
  ...
  const isTime =
    /target:\s*finish in a goal time/i.test(notes) || /goal type:\s*time goal/i.test(notes);
