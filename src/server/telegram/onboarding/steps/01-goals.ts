...
function parseDistance(text: string): ParseResult<Distance> {
  ...
  if (/^(full\s+)?marathon$/.test(v) || v === '26.2' || v === 'm' || v === 'full')
...
function parseTarget(text: string): ParseResult<Target> {
  ...
  return {
    ok: false,
    error:
      'Not sure what you mean — "time" if you\'re chasing a goal time, "finish" if you just want to cross the line.',
  };
}
...
function parseMeaning(text: string): ParseResult<string> {
  ...
  if (v.length > 500) return { ok: false, error: 'Keep it under 500 characters.' };
