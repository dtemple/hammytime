...
describe("renderBYOPlanTemplate", () => {
  ...
  it("throws on missing variable", async () => {
    ...
    await expect(
      renderBYOPlanTemplate(missingName as AnyMock)
    ).rejects.toThrow('missing variable "name"');
  });
...
function makeFullDb(overrides: {
  ...
  const planVersionInsert = vi.fn().mockResolvedValue({ data: { id: "version-1" }, error: null });
...
describe("handleBuildPath", () => {
  ...
  it("sends cover note + at least one template chunk via sendAndLog", async () => {
    const db = makeFullDb();
    (supabaseAdmin as AnyMock).mockReturnValue({ from: db.fromMock });
    ...
    // First call is the cover note
    expect(calls[0]![2]).toContain("paste it into Claude or ChatGPT");
  });
  ...
  it("fires sendDavidAlert with 'build path'", async () => {
    ...
    expect(sendDavidAlert).toHaveBeenCalledWith(
      expect.stringContaining("build path")
    );
  });
  ...
  it("advances onboarding state to terminal (step = 7)", async () => {
    ...
    expect(advanceQuestion).toHaveBeenCalledWith(
      "athlete-1",
...
describe("handleHelpPath", () => {
  it("sends athlete confirmation and David alert without creating plan rows", async () => {
    ...
    expect(sendDavidAlert).toHaveBeenCalledWith(
      expect.stringContaining("help path")
    );
