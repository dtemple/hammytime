...
function makeDb(
      ...
      weeks: [
        {
          ...
          phase: 'base',
          days: [
            { day: 'Monday', type: 'easy', description: 'Easy run', planned_distance_miles: 6 },
          ],
        },
      ],
    },
  ...
  const memoryEqChain = {
    ...
    maybeSingle: vi.fn().mockImplementation(() => {
      ...
      memoryFileCallCount++;
      const content = memoryFileCallCount === 1 ? injuryLogMd : checkinLogMd;
...
function makeAnthropicClient(
  responseText = 'You had a solid week. Easy 6 miles today at RPE 4–5. Hamstring — do 3×15 single-leg calf raises before you head out.',
) {
  return {
    messages: {
...
describe('runDailyCheckin', () => {
  it('returns telegramMessage and checkinLogEntry, persists agent_runs row', async () => {
    ...
    const [insertArg] = (db.agentRunsInsertMock as AnyMock).mock.calls[0] as [
...
describe('runDailyCheckin — Strava hard requirement', () => {
  it('broken token: throws StravaTokenBrokenError, agent_runs row recorded, Claude not called', async () => {
    ...
    (supabaseAdmin as AnyMock).mockReturnValue(db);
    (loadAthleteData as AnyMock).mockResolvedValue(ATHLETE_DATA);
    (fetchRecentActivities as AnyMock).mockRejectedValue(
      new Error('refresh_token rejected by Strava'),
    );
    const client = makeAnthropicClient();
    (anthropicClient as AnyMock).mockReturnValue(client);

    await expect(runDailyCheckin(ATHLETE_ID, WELLNESS)).rejects.toThrow(
      'Strava token refresh failed or was revoked',
    );
