...
// One normalized event per plan day, shared by the ICS feed (calendar-render.ts)
// and the Google Calendar writer (src/server/google/sync.ts). Both paths must
// emit the same event set; keeping the mapping in one place is what guarantees
// it (Specs/CALENDAR_OAUTH.md).
export type CalendarEvent = {
  // `${planId}-w${week}-d${dayIndex}@hammytime` — keyed on the plan id (stable
  // across versions), not the version id, so a coach edit that bumps the
  // working version keeps UIDs stable and only changed days move. Doubles as
  // the Google iCalUID.
  uid: string;
  date: string; // ISO yyyy-mm-dd — the all-day calendar date
  summary: string;
  description: string;
  location?: string; // 'trail' when day.prefer_trail
};
...
// Date-only arithmetic in UTC so the result can never cross a calendar day
// regardless of the server's timezone.
function isoPlusDays(iso: string, offsetDays: number): string {
...
function eventDate(
  ...
  return isoPlusDays(planStartDate, (week.week_number - 1) * 7 + dayIndex);
}
...
export function planToCalendarEvents(input: CalendarEventsInput): CalendarEvent[] {
  const { plan, planId, planStartDate } = input;
  if (!plan || !planId) return [];

  const events: CalendarEvent[] = [];
  for (const week of plan.weeks) {
    week.days.forEach((day, dayIndex) => {
      const date = eventDate(day, week, dayIndex, planStartDate);
      if (!date) return;
      events.push({
        uid: `${planId}-w${week.week_number}-d${dayIndex}@hammytime`,
        date,
        summary: summaryFor(day),
        description: descriptionFor(day, plan),
        ...(day.prefer_trail ? { location: 'trail' } : {}),
      });
    });
  }
  return events;
